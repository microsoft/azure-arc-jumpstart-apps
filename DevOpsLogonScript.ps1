...
Write-Header "Downloading Istio Binary"
Invoke-WebRequest -Uri "https://github.com/istio/istio/releases/download/$istioReleaseVersion/istio-$istioReleaseVersion-win-amd64.zip" -Outfile "$Env:TempDir\istio-$istioReleaseVersion-win-amd64.zip"
Expand-Archive "$Env:TempDir\istio-$istioReleaseVersion-win-amd64.zip" -DestinationPath $Env:TempDir
Copy-Item "$Env:TempDir\istio-$istioReleaseVersion\bin\istioctl.exe" -Destination $Env:ToolsDir
